...
void Render(void)
{
	...
	VECTOR3D point = interpolate(curve, t);
	drawDot(point, 0.1, red);
...
void HandleMouseMotion(int x, int y)
{
	euler.pitch = -(25.0 / (camera.screenheight / 2)) * (y - (camera.screenheight / 2)) * DTOR;
	euler.yaw = -(25.0 / (camera.screenwidth / 2)) * (x - (camera.screenwidth / 2)) * DTOR;
	euler.roll = 0;
}